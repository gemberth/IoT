...
cred = credentials.Certificate("esp32-iot-54d37-firebase-adminsdk-1c777-8fffaf3d3a.json")
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://esp32-iot-54d37-default-rtdb.firebaseio.com/'
})
...
def obtener_datos():
    ...
    ref = db.reference('UsersData/6jFdOa9XmBeI9ZnihfOCahSYJZh1')

    # Obtener los datos de la referencia
    datos = ref.get()

    # Extraer los valores de la clave "fecha"
    fechas = datos.get("readings",{})

    # Validar si no hay datos
    if not fechas:
        advertencia = "No hay datos disponibles en la base de datos."
        return render_template('datos.html', advertencia=advertencia)

    # Crear un archivo CSV y escribir los datos en él
    with open('datos.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Fecha', 'Temperatura', 'Humedad'])  # Escribir la fila de encabezado

        for fecha, valores in fechas.items():
            fecha_dt = datetime.fromtimestamp(int(fecha))
            temperature = valores.get('temperature', '')
            humidity = valores.get('humidity', '')
            writer.writerow([fecha_dt, temperature, humidity])

    # Leer el archivo CSV y crear una respuesta para descargarlo
...
@app.route('/datosecxel', methods=['GET'])
def obtener_datoss():
    # Obtener una referencia a la base de datos
    ref = db.reference('UsersData/6jFdOa9XmBeI9ZnihfOCahSYJZh1')
    ...
    for fecha, valores in fechas.items():
        ...
        temperature = valores.get('temperature', '')
        humidity = valores.get('humidity', '')
        data.append([fecha_dt, temperature, humidity])
    
    df = pd.DataFrame(data, columns=['Fecha', 'Temperatura', 'Humedad'])

    # Guardar el DataFrame en un archivo Excel
    df.to_excel('datosEcxel.xlsx', index=False)

    # Leer el contenido del archivo Excel
    with open('datosEcxel.xlsx', 'rb') as file:
        contenido_excel = file.read()

    # Crear una respuesta con el contenido del archivo Excel como descarga
    response = make_response(contenido_excel)

    # Establecer las cabeceras para la descarga del archivo Excel
    response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    response.headers['Content-Disposition'] = 'attachment; filename=datosEcxel.xlsx'
    response.headers.add('Access-Control-Allow-Origin', '*')
...
def calcularMontecarlo():
 ...
 # Suma de Pagos a tenedor
    ...
    datapro= dfMCL['Pagos a tenedor'].mean()
    dataArray.append(data)
    dataArray.append(datapro)
